...
const EventDetails = () => {
  ...
  const handleCancelRegistration = async () => {
    ...
    if (!userId || !token || cancelLoading || !isRegistered) {
      ...
      if (!token) showMessage('Authentication token not found. Please login again.');
      return;
    }
    ...
    try {
      const res = await apiCall(`${API_BASE_URL}/api/events/${id}/cancel-registration`, { 
        method: 'PATCH',
        body: JSON.stringify({
  ...
  const handleFeedback = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showMessage(res.status === 404 ? 'Feedback form not available yet' : 'Failed to load feedback form');
        return;
      }
        ...
        <main className="flex-1 w-full px-2 sm:px-8 py-8 with-sidebar mt-24">
          {showPopup && (
            <div className="fixed top-8 left-1/2 transform -translate-x-1/2 z-50">
              <div className={`px-6 py-4 rounded-xl shadow-lg text-lg font-semibold border ${
                ...
                popupMessage.includes('Already') ? 'bg-blue-600 text-white' :
                popupMessage.includes('cancelled') ? 'bg-orange-600 text-white' :
          ...
          {showCancelModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Cancel Registration</h3>
                <p className="text-gray-600 mb-6">Please tell us why you're cancelling your registration:</p>
                ...
                {cancellationReason === 'Other' && (
                  <div className="mb-6">
                    <textarea
                      ...
                      maxLength="200"
                    />
                    <p className="text-sm text-gray-500 mt-1">{customReason.length}/200 characters</p>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={cancelLoading || !cancellationReason || (cancellationReason === 'Other' && !customReason.trim())}
          ...
          {loading ? (
            ...
            <div className="text-center mt-20">
              ...
              <button onClick={fetchData} className="bg-blue-600 text-white px-6 py-3 rounded-lg">Retry</button>
            </div>
          ) : (
            <>
              {/* Event Image Banner */}
              <div className="relative rounded-2xl overflow-hidden shadow-lg mb-10">
                <img
                  src={event.banner_image?.trim() ? event.banner_image : defaultEvent}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-8">
                  ...
                  <span className="inline-block bg-white/90 text-gray-800 px-4 py-2 rounded-full text-base font-semibold mb-2">{event.type}</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8 mb-10">
                ...
                <div className="text-gray-700 text-lg mb-4">Time: {formatTime(event.start_time)} - {formatTime(event.end_time)}</div>
                {event.type?.toLowerCase() === 'job fair' && <div className="text-gray-700 text-lg mb-4">Location: {event.location}</div>}
                  ...
                  {!isRegistered ? (
                    <button
                      ...
                      className={`px-6 py-3 rounded-lg font-semibold text-white transition ${
                        registerLoading ? 'bg-gray-500' : 'bg-red-900 hover:bg-red-800'
                      }`}
                    >
                    ...
                    <>
                      <button disabled className="px-6 py-3 rounded-lg font-semibold text-white bg-green-500">Applied ✓</button>
                      <button
                        onClick={handleCancelClick}
                        disabled={cancelLoading}
                        className={`px-6 py-3 rounded-lg font-semibold text-white transition ${
                          cancelLoading ? 'bg-gray-500' : 'bg-orange-600 hover:bg-orange-700'
                        }`}
                      >
                        Cancel Registration
                      </button>
                ...
                {(event.slido_link || event.slido_embed_url) ? (
                  ...
                  <div className="mt-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                    <h3 className="text-lg font-semibold text-blue-800 mb-3">💬 Ask Questions Live</h3>
                    <p className="text-blue-700 mb-4">Join the conversation and ask your questions during the event!</p>
                    <div className="text-blue-600 font-medium">📋 Slido will be available before the event starts</div>
                  </div>
                )}
              </div>
