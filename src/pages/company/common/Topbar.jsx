...
const Topbar = () => {
  ...
  useEffect(() => {
    const fetchLatestPublishedJobFair = async () => {
      try {
        ...
        const publishedFairs = allFairs.filter(
          fair => fair.status === 'published'
        );
        ...
        setCompanyData(prev => ({
          ...
          logo_path: data.logo_path
            ? data.logo_path.startsWith('http')
              ? data.logo_path
              : `${APP_URL}/storage/${data.logo_path}`
            : '',
  ...
  const navItems = [
    { label: 'Profile', path: 'profile', show: true },
    {
      ...
      path: `job-fairs/${jobFairId}/setup`,
      show: !!jobFairId,
    },
    {
      label: 'Requests',
      path: `job-fairs/${jobFairId}/requests`,
      show: isApproved && !!jobFairId,
    },
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `group relative px-6 py-3 rounded-2xl transition-all duration-300 transform hover:-translate-y-0.5 tracking-wide overflow-hidden ${
                  isActive
                    ? 'text-[#901b20] bg-white/10 backdrop-blur-3xl shadow-2xl shadow-[#901b20]/90'
