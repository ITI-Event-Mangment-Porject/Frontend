    ...
    const fetchCompany = async () => {
      ...
      try {
        const response = await axios.get(
          `${APP_URL}/api/companies/${companyId}`,
          {
            headers: {
        ...
        setCompanyData({
          ...
          logo_path: result.logo_path
            ? result.logo_path.startsWith('http')
              ? result.logo_path
              : `${APP_URL}/storage/${result.logo_path}`
            : '',
        });
      } catch (err) {
  ...
  const handleLogoUpload = async e => {
    ...
    try {
      const response = await axios.post(
        `${APP_URL}/api/companies/${companyId}/logo`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
                    ...
                    <>
                      ...
                      >
                        {loading ? (
                          ...
                          <Save className="w-5 h-5 transition-transform duration-300 group-hover/btn:scale-110" />
                        )}
