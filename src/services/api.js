...
export const eventAPI = {
  ...
  startEvent: (id) => {
    console.log("eventAPI.startEvent - Token:", localStorage.getItem("token"))
    console.log("eventAPI.startEvent - Event ID:", id)
    return api.post(`/test/live/events/${id}/start`)
  },
  endEvent: (id) => {
    console.log("eventAPI.endEvent - Token:", localStorage.getItem("token"))
...
export const messageAPI = {
  ...
  getAllMessages: (params) => {
    console.log("messageAPI.getAllMessages - Token:", localStorage.getItem("token"))
    return api.get("/message/bulk-message", { params })
  },
  getStats: () => {
  ...
  sendBulkMessages: (messageData) => {
    console.log("messageAPI.sendBulkMessages - Token:", localStorage.getItem("token"))
    return api.post("/message/bulk-message", messageData)
  }, // Keep for backward compatibility
  sendMessage: (id) => {
    console.log("messageAPI.sendMessage - Token:", localStorage.getItem("token"))
...
export const jobFairAPI = {
  getParticipations: (jobFairId) => {
    ...
    return api.get(`/job-fairs/${jobFairId}/participations`).catch((error) => {
      console.error("Error fetching job fair participations:", error.response?.status, error.response?.data)
      if (error.response?.status === 401) {
        console.warn(
  ...
  getCompanyQueue: (eventId, companyId) => {
    ...
    return api.get(`/job-fairs/${eventId}/queues/company/${companyId}`).catch((error) => {
      console.error("Error fetching company queue:", error.response?.status, error.response?.data)
      return Promise.reject(error)
    })
  },
  getParticipationsWithFallback: (jobFairId) => {
    ...
    return api.get(`/job-fairs/${jobFairId}/participations`).catch((error) => {
      console.warn("Using fallback data for job fair participations due to API error:", error.message)
  ...
  getAllParticipations: async (eventIds) => {
    ...
    try {
      ...
      const requests = ids.map((id) => api.get(`/job-fairs/${id}/participations`))
  ...
  getAllParticipationsWithFallback: async (eventIds) => {
    try {
      ...
      console.warn("Using fallback data for all job fair participations due to API error:", error.message)
  ...
  getParticipationDetails: (jobFairId, participationId) => {
    if (!jobFairId || !participationId) {
      console.error("Missing jobFairId or participationId in getParticipationDetails call")
      return Promise.reject(new Error("Job Fair ID and Participation ID are required"))
    }

    console.log("jobFairAPI.getParticipationDetails - Token:", localStorage.getItem("token"))
    console.log("jobFairAPI.getParticipationDetails - Job Fair ID:", jobFairId, "Participation ID:", participationId)

    return api.get(`/job-fairs/${jobFairId}/participations/${participationId}`).catch((error) => {
      console.error("Error fetching participation details:", error.response?.status, error.response?.data)
      return Promise.reject(error)
    })
  },
  ...
  updateParticipationStatus: (jobFairId, participationId, status) => {
    if (!jobFairId || !participationId || !status) {
      console.error("Missing required parameters in updateParticipationStatus call")
      return Promise.reject(new Error("Job Fair ID, Participation ID, and status are required"))
    }

    console.log("jobFairAPI.updateParticipationStatus - Token:", localStorage.getItem("token"))
    console.log(
    ...
    return api
      ...
      .catch((error) => {
        console.error("Error updating participation status:", error.response?.status, error.response?.data)
        return Promise.reject(error)
      })
  },
...
export const attendanceAPI = {
  getReports: () => {
    console.log("attendanceAPI.getReports - Token:", localStorage.getItem("token"))
    return api.get("/reports/attendance")
  },

  getEventAttendance: (eventId) => {
    console.log("attendanceAPI.getEventAttendance - Token:", localStorage.getItem("token"))
  ...
  exportAttendance: (format = "xlsx", eventId = null) => {
    console.log("attendanceAPI.exportAttendance - Token:", localStorage.getItem("token"))
    console.log("attendanceAPI.exportAttendance - Format:", format, "Event ID:", eventId)
  ...
  getAttendanceStats: () => {
    console.log("attendanceAPI.getAttendanceStats - Token:", localStorage.getItem("token"))
