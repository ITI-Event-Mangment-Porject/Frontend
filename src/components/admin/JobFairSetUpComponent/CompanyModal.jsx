        ...
        <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 overflow-y-auto">
          ...
          <div className="px-6 py-4">
            <div className="flex items-center mb-4">
              {company.logo_path ? (
                <img className="h-16 w-16 rounded-full" src={company.logo_path} alt={company.name} />
              ) : (
                <div className="h-16 w-16 bg-gray-200 rounded-full flex items-center justify-center">
                  <span className="text-gray-500 text-xl">{company.name.charAt(0)}</span>
                </div>
              )}
              <div className="ml-4">
            ...
            <p className="text-sm text-gray-600 mb-4">Description: {company.description}</p>
  
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h5 className="font-medium text-gray-900 mb-2">Contact Information</h5>
                <p className="text-sm text-gray-600">Email: {company.contact_email}</p>
                <p className="text-sm text-gray-600">Phone: {company.contact_phone}</p>
                <p className="text-sm text-gray-600">Location: {company.location}</p>
                <div className="flex gap-2 mt-3">
                  <a href={company.website} target="_blank" rel="noopener noreferrer"
                    className="text-sm bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">Website</a>
                  <a href={company.linkedin_url} target="_blank" rel="noopener noreferrer"
                    className="text-sm bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">LinkedIn</a>
                </div>
              </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200">
            <button onClick={onClose} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">Close</button>
          </div>
        </div>
      </div>
    );
