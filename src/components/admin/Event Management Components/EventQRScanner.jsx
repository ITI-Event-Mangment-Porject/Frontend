...
export default function EventQRScanner({ event }) {
    ...
    const fetchAttendees = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setAttendees(data.data.result || []);
          setCheckedInCount(data.data.result?.length || 0);
        } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newAttendee = {
          ...
          name: data.data?.user_name || 'Unknown',
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 via-white to-gray-100 p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="text-center space-y-4 animate-fade-in">
          <h1 className="text-4xl sm:text-5xl font-black bg-gradient-to-r from-[#901b20] via-[#ad565a] to-[#cc9598] bg-clip-text text-transparent mb-4">
            Student Check-In for "{event.title}"
          </h1>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  ...
                  <th className="py-3 px-4 font-bold text-gray-600">Email</th>
                  <th className="py-3 px-4 font-bold text-gray-600">
                    Check-in Time
                  </th>
                </tr>
              </thead>
              <tbody>
                {attendees.map(attendee => (
                  <tr key={attendee.Email} className="border-b border-gray-100">
